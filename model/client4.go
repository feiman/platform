...
func BuildResponseMetadata(r *http.Response) *ResponseMetadata {
...
func (c *Client4) LoginById(id string, password string) (*User, *ResponseMetadata) {
...
func (c *Client4) Login(loginId string, password string) (*User, *ResponseMetadata) {
	m := make(map[string]string)
	m["login_id"] = loginId
	m["password"] = password
	return c.login(m)
...
func (c *Client4) LoginByLdap(loginId string, password string) (*User, *ResponseMetadata) {
	m := make(map[string]string)
	m["login_id"] = loginId
	m["password"] = password
	m["ldap_only"] = "true"
...
// the session.
func (c *Client4) LoginWithDevice(loginId string, password string, deviceId string) (*User, *ResponseMetadata) {
	m := make(map[string]string)
	m["login_id"] = loginId
	m["password"] = password
...
func (c *Client4) login(m map[string]string) (*User, *ResponseMetadata) {
	if r, err := c.DoApiPost("/users/login", MapToJson(m)); err != nil {
		...
		c.AuthType = HEADER_BEARER
		defer closeBody(r)
		return UserFromJson(r.Body), BuildResponseMetadata(r)
	}
}

// Logout terminates the current user's session.
func (c *Client4) Logout() (bool, *ResponseMetadata) {
	if r, err := c.DoApiPost("/users/logout", ""); err != nil {
		...
		defer closeBody(r)
		return CheckStatusOK(r), BuildResponseMetadata(r)
	}
}
...
func (c *Client4) CreateUser(user *User) (*User, *ResponseMetadata) {
...
func (c *Client4) GetUser(userId, etag string) (*User, *ResponseMetadata) {
...
func (c *Client4) UpdateUser(user *User) (*User, *ResponseMetadata) {
